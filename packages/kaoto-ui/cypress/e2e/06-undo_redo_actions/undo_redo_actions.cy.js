describe('Test for undo/redo actions on code-editor', () => {
  ...
  it.skip("undoes and redoes the user's changes, updating the visualization respectively", () => {
    ...
    cy.isomorphicGet('[data-testid="viz-step-kafka-source"]').should('be.visible');

    // LOAD SECOND FIXTURE
    cy.uploadFixture('ChuckNorris.yaml', false);

    // CHECK THE VISUALIZATION CANVAS HAS UPDATED ACCORDINGLY
    cy.isomorphicGet('[data-testid="viz-step-chuck-norris-source"]').should('be.visible');
    cy.isomorphicGet('[data-testid="viz-step-chunk-template-action"]').should('be.visible');
    cy.isomorphicGet('[data-testid="viz-step-kafka-sink"]').should('be.visible');
