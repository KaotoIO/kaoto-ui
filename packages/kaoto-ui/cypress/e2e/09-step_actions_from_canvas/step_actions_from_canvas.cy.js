describe('Test for Step actions from the canvas', () => {
    ...
    it(' User inserts a step between two steps (+ button in between two nodes)', () => {
        ...
        cy.isomorphicGet('[data-testid="viz-step-aggregate"]').should('be.visible');
    ...
    it('In an integration with at least two steps, user deletes the first step, showing a placeholder step in its place (start-end)', () => {
        ...
        cy.isomorphicGet('.stepNode').should('have.length', 2);
        // CHECK that stepNodes are in the correct order
        cy.isomorphicGet('.stepNode').eq(0).should('have.attr', 'data-testid', 'viz-step-slot');
        cy.isomorphicGet('.stepNode').eq(1).should('have.attr', 'data-testid', 'viz-step-log');
    ...
    it('In an integration with at least two steps, user deletes the first step, showing a placeholder step in its place (start-action)', () => {
        ...
        // CHECK that stepNodes are in the correct order
        cy.isomorphicGet('.stepNode').eq(0).should('have.attr', 'data-testid', 'viz-step-slot');
        cy.isomorphicGet('.stepNode').eq(1).should('have.attr', 'data-testid', 'viz-step-arangodb');
        cy.isomorphicGet('.stepNode').eq(2).should('have.attr', 'data-testid', 'viz-step-log');
        // CHECK that stepNodes contains of the two steps
        cy.isomorphicGet('.stepNode').should('have.length', 3);
        // CHECK that YAML not contains the 'timer:null'
    ...
    it('Step Detail - User configures a normal step, which updates the YAML)', () => {
        ...
        cy.isomorphicGet('[name="logMask"]').click();
        ...
        cy.isomorphicGet('.code-editor').should('not.contain.text', 'logMask');

        // Change the value of the "groupDelay" integer field
        cy.isomorphicGet('[name="groupDelay"]').clear().type('15000');
    ...
    it('Step detail - User selects a step, which opens the detail drawer', () => {
        ...
        cy.deleteStep('log');
        cy.isomorphicGet('[data-testid="kaoto-right-drawer"]').should('not.be.visible');
