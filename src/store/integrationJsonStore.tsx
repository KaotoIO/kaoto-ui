  ...
  temporal(
    (set, get) => ({
      ...
      deleteBranchStep: (newRootStep: IStepProps, rootStepIndex: number) => {
        ...
        const stepsWithNewUuids = StepsService.regenerateUuids(get().integrationJson.id, newSteps);
      ...
      deleteStep: (stepIdx) => {
        ...
        const stepsWithNewUuids = StepsService.regenerateUuids(get().integrationJson.id, updatedSteps);
      ...
      replaceBranchParentStep: (newParentStep, pathToParentStep) => {
        ...
        const stepsWithNewUuids = StepsService.regenerateUuids(get().integrationJson.id, stepsCopy);
        const { updateSteps } = useNestedStepsStore.getState();
        updateSteps(StepsService.extractNestedSteps(stepsWithNewUuids));
      ...
      updateIntegration: (newInt: IIntegration) => {
        ...
        const uuidSteps = StepsService.regenerateUuids(get().integrationJson.id, newIntegration.steps);
        const updateSteps = useNestedStepsStore.getState().updateSteps;
